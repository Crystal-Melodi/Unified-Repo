...
struct SoundOpenHandler : FileSystem::OpenHandler
{
	...
	bool tryRead(SDL_RWops &ops, const char *ext)
	{
		...
		if (!strcmp(sig, "OggS"))
		{
			SDL_RWseek(&ops, 28, RW_SEEK_SET);
			char head[9] = { 0 };
			SDL_RWread(&ops, head, 1, 8);
			SDL_RWseek(&ops, 0, RW_SEEK_SET);

			if(!strcmp(head, "OpusHead"))
			{
				int error;
        		OggOpusFile* of = op_open_callbacks(&ops, &opCallbacks, nullptr, 0, &error);
				...
				int pcm = op_pcm_total(of, -1);
				int16_t *buf = new int16_t[pcm*2];
				int readed = 0;

				while(pcm > readed)
				{
					int res = op_read_stereo(of, buf+readed*2, pcm*2);
				...
				buffer->bytes = readed * 4;

				AL::Buffer::uploadData(buffer->alBuffer, AL_FORMAT_STEREO16, buf,
							   		   buffer->bytes, 48000);
