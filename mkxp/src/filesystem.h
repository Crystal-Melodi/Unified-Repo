...
public:
	...
	void createOrLoadPathCache(bool pathCache, bool prebuilt, bool fastPathEnum);
	...
	//Find case-sensitive dir path
	std::string findDirPath(const char* path);
    
    //Save path cache to disk
    void savePathCache(bool pathCacheEnabled);
    void saveFileLists();
    //Load path cache from disk
    bool loadPathCache(bool pathCacheEnabled);
    bool loadFileLists();
private:
	FileSystemPrivate *p;
};
