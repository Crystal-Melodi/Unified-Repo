...
static void setupSplash(SDL_Window *win)
{
    SDL_RWops *splashSrc;
    splashSrc = SDL_RWFromConstMem(mkxp_assets_splash_jpg, mkxp_assets_splash_jpg_len);
    
    SDL_Surface *screen;
    screen = SDL_GetWindowSurface(win);
    SDL_Surface *splash;
    splash = IMG_Load_RW(splashSrc, SDL_TRUE);
    
    SDL_BlitScaled(splash, NULL, screen, NULL);
...
//Run init code for android
    ...
    {
		Debug()<<"Game Folder: "<<conf.gameFolder.c_str();
	...
	{
		Debug()<<"gameFolder is empty.";
	}

	conf.readGameINI();

	if (conf.windowTitle.empty())
		conf.windowTitle = "RPG Maker Plugin for JoiPlay";
