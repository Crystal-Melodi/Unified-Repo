...
struct GraphicsPrivate
{
	...
	void recalculateScreenSize(RGSSThreadData *rtData)
	{
    	...
    	/* Set screen alignment when screen orientation is portrait */
		if(threadData->config.verticalScreenAlign == "center")
		{
			scOffset.y = (winSize.y - scSize.y) / 2.f;
		}
		else if(threadData->config.verticalScreenAlign == "top")
		{
			scOffset.y = winSize.y > winSize.x ? (winSize.y - scSize.y) : (winSize.y - scSize.y) / 2.f;
		}
		else if(threadData->config.verticalScreenAlign == "top-center")
		{
			if(winSize.y > winSize.x)
			{
				scOffset.y = (winSize.y * 0.7f) > scSize.y ? (winSize.y - scSize.y - (((winSize.y * 0.7f) - scSize.y) / 2.f)) : (winSize.y - scSize.y);
			}
			else
			{
				scOffset.y = (winSize.y - scSize.y) / 2.f;
	...
	void redrawScreen()
	{
		...
		{
			char fpsBuff[16];
			snprintf(fpsBuff, sizeof(fpsBuff), "FPS: %.2f", averageFPS());

			Bitmap tmpBitmap = Bitmap(64, 64);
			tmpBitmap.getFont().setSize(12);
			IntRect textSize = tmpBitmap.textSize(fpsBuff);

			Bitmap fpsBitmap = Bitmap(textSize.w, textSize.h);
			fpsBitmap.getFont().setSize(12);
			fpsBitmap.fillRect(IntRect(0, 0, textSize.w, textSize.h), Vec4(0,0,0,255));
			fpsBitmap.drawText(0, 0, textSize.w, textSize.h, fpsBuff, 0);

			GLMeta::blitBegin(screen.getPP().frontBuffer());
			GLMeta::blitSource(fpsBitmap.getGLTypes());
			GLMeta::blitRectangle(textSize, Vec2i());
			GLMeta::blitEnd();
		}
...
void Graphics::update(bool checkForShutdown)
{
    ...
    if (p->threadData->config.fastForwardSpeed > 0 && p->threadData->config.fastForward && p->supportFastForward)
    {
...
void Graphics::disableFastForward()
{
    p->supportFastForward = false;
}

void Graphics::showFPS(bool enabled)
{
	p->showFPS = enabled;
}
