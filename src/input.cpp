...
std::map<std::string, int> strToScancode{
...
bool Input::isReleasedEx(int code, bool isVKey)
{
    ...
    return p->getStateRaw(c).released;
}
...
int Input::mouseX()
{
    ...
    int x = (EventThread::mouseState.x - rtData.screenOffset.x) * rtData.sizeResoRatio.x;
	return x > 0 ? x:0;
}
...
{
	RGSSThreadData &rtData = shState->rtData();
	int y = (EventThread::mouseState.y - rtData.screenOffset.y) * rtData.sizeResoRatio.y;
	return y > 0 ? y:0;
}
