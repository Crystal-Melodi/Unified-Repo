...
bool INIConfiguration::Section::getIntProperty (const std::string& name, int& outPropInt) const
{
	try
	{
		std::string prop = m_PropertyMap.at(toLowerCase(name)).m_Value;
		outPropInt = std::stoi(prop);
		return true;
	}
	catch (std::out_of_range& oorexcept)
	{
...
int INIConfiguration::getIntProperty(const std::string& sname, const std::string& name, const int& def) const
{
	auto sectionIt = m_SectionMap.find(toLowerCase(sname));
	...
	if (sectionIt != m_SectionMap.end())
	{
		...
		if(sectionIt->second.getIntProperty(name, prop))
		{
			return prop;
		}
	}
