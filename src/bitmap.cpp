...
struct BitmapOpenHandler : FileSystem::OpenHandler
{
	...
	BitmapOpenHandler()
    : surface(0), animation(0), gif_data(0), gif_data_size(0)
	{}
	...
	bool tryRead(SDL_RWops &ops, const char *ext)
	{
        if (IMG_isGIF(&ops)) {
            ...
            gif_create(animation, &gif_bitmap_callbacks);
            ...
            int status;
            do {
                status = gif_initialise(animation, gif_data_size, gif_data);
                if (status != GIF_OK && status != GIF_WORKING) {
                    gif_finalise(animation);
                    delete animation;
                    delete gif_data;
            ...
            status = gif_decode_frame(animation, 0);
            if (status != GIF_OK && status != GIF_WORKING) {
...
Bitmap::Bitmap(const char *filename)
{
    ...
    if (!handler.animation && !handler.surface) {
		//Ignore the exception
        //throw Exception(Exception::SDLError, "Error loading image '%s': %s",
        //                filename, SDL_GetError());
		Debug()<<"Error loading image "<<filename<<" : "<<SDL_GetError();
    }
    
    if (handler.animation) {
        p = new BitmapPrivate(this);

        if (handler.animation->width >= glState.caps.maxTexSize || handler.animation->height > glState.caps.maxTexSize)
        {
            throw new Exception(Exception::MKXPError, "Animation too large (%ix%i, max %ix%i)",
                                handler.animation->width, handler.animation->height, glState.caps.maxTexSize, glState.caps.maxTexSize);
        }
        
        if (handler.animation->frame_count == 1) {
            TEXFBO texfbo;
            try {
                texfbo = shState->texPool().request(handler.animation->width, handler.animation->height);
            }
            catch (const Exception &e)
            {
                gif_finalise(handler.animation);
                delete handler.animation;
            ...
            TEX::bind(texfbo.tex);
			TEX::uploadImage(handler.animation->width, handler.animation->height, handler.animation->frame_image, GL_RGBA);
			gif_finalise(handler.animation);
            delete handler.animation;
            delete handler.gif_data;
        ...
        p->animation.width = handler.animation->width;
        p->animation.height = handler.animation->height;

        // Guess framerate based on the first frame's delay
        p->animation.fps = 1 / ((float)handler.animation->frames[handler.animation->decoded_frame].frame_delay / 100);
        if (p->animation.fps < 0) p->animation.fps = shState->graphics().getFrameRate();
        
        // Loop animation (IMG_Animation doesn't have loop information so loop all animations)
        p->animation.loop = handler.animation->loop_count >= 0;
        
		int fcount = handler.animation->frame_count;
        int fcount_partial = handler.animation->frame_count_partial;
        ...
        for (int i = 0; i < fcount_partial; i++) {
			if (i > 0) {
                int status = gif_decode_frame(handler.animation, i);
                if (status != GIF_OK && status != GIF_WORKING) {
            ...
            TEX::bind(texfbo.tex);
            TEX::uploadImage(p->animation.width, p->animation.height, handler.animation->frame_image, GL_RGBA);
	...
	if (!imgSurf)
	{
		//Just create a 48x48 bitmap instead of throwing an exception
		//throw Exception(Exception::SDLError, "Error loading image '%s': %s",
		//                filename, SDL_GetError());
       
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
        imgSurf = SDL_CreateRGBSurface(SDL_SWSURFACE,48,48,32, 0xFF000000, 0x00FF0000, 0x0000FF00, 0x000000FF);
#else
        imgSurf = SDL_CreateRGBSurface(SDL_SWSURFACE,48,48,32, 0x000000FF, 0x0000FF00, 0x00FF0000, 0xFF000000);
#endif
...
{
	TEXFBO tex;
	if ((width <= 0 || height <= 0) || (width > glState.caps.maxTexSize || height > glState.caps.maxTexSize))
	{
		tex = shState->texPool().request(4, 4);
	}
	else
	{
...
Bitmap::Bitmap(const Bitmap &other, int frame)
{
	...
	if(other.megaSurface())
	{
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
        p->megaSurface = SDL_CreateRGBSurface(SDL_SWSURFACE,other.width(),other.height(),32, 0xFF000000, 0x00FF0000, 0x0000FF00, 0x000000FF);
#else
        p->megaSurface = SDL_CreateRGBSurface(SDL_SWSURFACE,other.width(),other.height(),32, 0x000000FF, 0x0000FF00, 0x00FF0000, 0xFF000000);
#endif
		SDL_BlitSurface(other.megaSurface(), NULL, p->megaSurface, NULL);
		p->addTaintedArea(rect());
	}
	else
	{
                        ...
                        int opacity)
{
	...
	if(p->megaSurface)
	{
		Debug()<<"Bitmap::stretchBlt is not supported on mega surfaces";
		if(srcSurf)
		{
			SDL_Rect srcRect = sourceRect;
			SDL_Rect dstRect = destRect;
			SDL_BlitSurface(srcSurf, &srcRect, p->megaSurface, &dstRect);
...
void Bitmap::fillRect(const IntRect &rect, const Vec4 &color)
{
	...
	if(p->megaSurface)
	{
		...
		SDL_FillRect(p->megaSurface, &destRect, SDL_MapRGBA(p->format,color.x*255,color.y*255,color.z*255,color.w*255));
		p->onModified();
		return;
	}
                              ...
                              bool vertical)
{
	...
	if(p->megaSurface)
	{
		Debug()<<"Bitmap::gradientFillRect is not supported on mega surfaces";
		return;
	}
...
void Bitmap::clearRect(const IntRect &rect)
{
	...
	if(p->megaSurface)
	{
		Debug()<<"Bitmap::clearRect is not supported on mega surfaces";
		return;
	}
...
void Bitmap::blur()
{
	...
	if(p->megaSurface)
	{
		Debug()<<"Bitmap::blur is not supported on mega surfaces";
		return;
	}
...
void Bitmap::radialBlur(int angle, int divisions)
{
	...
	if(p->megaSurface)
	{
		Debug()<<"Bitmap::radialBlur is not supported on mega surfaces";
		return;
	}
...
void Bitmap::clear()
{
	...
	if(p->megaSurface)
	{
		SDL_FillRect(p->megaSurface, NULL, 0x000000);
		p->clearTaintedArea();
...
Color Bitmap::getPixel(int x, int y) const
{
	...
	if(p->megaSurface)
	{
		Debug()<<"Bitmap::getPixel is not supported on mega surfaces";
...
void Bitmap::setPixel(int x, int y, const Color &color)
{
	...
	if(p->megaSurface)
	{
		Debug()<<"Bitmap::setPixel is not supported on mega surfaces";
		return;
	}
...
void Bitmap::saveToFile(const char *filename)
{
    ...
    std::string fn_normalized = shState->config().gameFolder+"/"+shState->fileSystem().normalizePath(filename);
    IMG_SavePNG(surf, fn_normalized.c_str());
    SDL_FreeSurface(surf);
}
...
void Bitmap::hueChange(int hue)
{
	...
	if(p->megaSurface)
	{
		Debug()<<"Bitmap::hueChange is not supported on mega surfaces";
		return;
	}
...
void Bitmap::drawText(const IntRect &rect, const char *str, int align)
{
	...
	if(p->megaSurface)
	{
		Debug()<<"Bitmap::drawText is not supported on mega surfaces";
		return;
	}
...
IntRect Bitmap::textSize(const char *str)
{
	...
	if(p->megaSurface)
	{
		Debug()<<"Bitmap::textSize is not supported on mega surfaces";
...
{
	guardDisposed();
    if (!p->animation.enabled || !p->animation.playing) return;
...
{
	guardDisposed();
    if (!p->animation.enabled || !p->animation.playing)
        return false;
