	...
	bool tryRead(SDL_RWops &ops, const char *ext)
	{
		...
		*srcOps = ops;
		...
		try
		{
			...
			if (!strcmp(sig, "OggS"))
			{
				SDL_RWseek(srcOps, 28, RW_SEEK_SET);
				char head[9] = { 0 };
				SDL_RWread(srcOps, head, 1, 8);
				SDL_RWseek(srcOps, 0, RW_SEEK_SET);

				if(!strcmp(head, "OpusHead"))
				{
					source = createOpusSource(*srcOps, looped);
				}
				else
				{
			...
			if (!strcmp(sig, "MThd"))
			{
                //blank midi files freeze games
                Sint64 size = srcOps->size(srcOps);
            ...
            if (!strcmp(sig, "RIFF"))
            {
                ...
                if((wavHeader.bitsPerSample ==0) || (wavHeader.bitsPerSample > 16))
                {
                    return false;
                }
            }
